  ...
  return createPortal(
    ...
    >
      ...
      <div
        ...
        className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-md bg-white shadow-lift dark:bg-jade-900 sm:rounded-md"
      >
        ...
        <div className="space-y-4 p-5">
          <div className="space-y-2 text-sm text-muted">
            <p className="flex items-start gap-2">
              <MapPin size={16} className="mt-0.5 shrink-0 text-subtle" /> {item.address}
            </p>
            {item.owner && (
              <p className="flex items-center gap-2">
                <User size={16} className="shrink-0 text-subtle" /> Đại diện: {item.owner}
              </p>
            )}
            {item.priceRange && (
              <p className="flex items-center gap-2">
                <Tag size={16} className="shrink-0 text-subtle" /> Giá tham khảo: {item.priceRange}
              </p>
            )}
          </div>
          ...
          {item.amenities?.length > 0 && (
            <div>
              <p className="mb-2 text-sm font-semibold text-body">Tiện nghi</p>
              <div className="flex flex-wrap gap-2">
                {item.amenities.map((a) => (
                  <span
                    key={a}
                    className="inline-flex items-center gap-1 rounded-md bg-jade-50 px-3 py-1 text-xs text-jade-700 dark:bg-jade-800 dark:text-jade-100"
                  >
                    <CheckCircle2 size={13} className="text-muted" /> {a}
                  </span>
                ))}
              </div>
            </div>
