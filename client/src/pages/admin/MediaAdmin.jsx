    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-jade-500">{items.length} ảnh · tối đa 8MB mỗi ảnh, tự nén sang WebP.</p>
        </div>
              ...
              <div className="p-3">
                {editAlt === m.id ? (
                  <div className="flex items-center gap-1">
                    <input value={altText} onChange={(e) => setAltText(e.target.value)} className="w-full rounded-lg border border-jade-200 px-2 py-1 text-xs outline-none dark:border-jade-700 dark:bg-jade-900" placeholder="Mô tả ảnh…" />
                    ...
                    <button onClick={() => setEditAlt(null)} className="grid h-7 w-7 place-items-center rounded text-jade-400"><X size={13} /></button>
                  </div>
                ) : (
                  <button onClick={() => { setEditAlt(m.id); setAltText(m.alt || ''); }} className="block w-full truncate text-left text-xs text-jade-500 hover:text-jade-700">
                    {m.alt || m.filename}
                  </button>
                )}
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-[10px] text-jade-400">{formatDate(m.createdAt)}</span>
                  <button onClick={() => del(m.id)} className="grid h-7 w-7 place-items-center rounded-lg text-red-500 hover:bg-red-50"><Trash2 size={13} /></button>
                </div>
              </div>
            </div>
          ))}
