...
const TONE = {
  ...
  terra: 'bg-terra-500/10 ring-terra-500/30 text-ink dark:bg-terra-500/15 dark:text-jade-50',
  ...
  return (
    <div>
      ...
      <section className="mt-6">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-muted">Việc cần làm</h2>
        {conViec.length === 0 ? (
          // Nói rõ là đã xong, không để một khoảng trống câm khiến người dùng
          // tưởng khối này hỏng.
          <p className="flex items-center gap-2 rounded-md bg-jade-50 p-4 text-sm text-jade-700 ring-1 ring-jade-200 dark:bg-jade-900/40 dark:text-jade-200 dark:ring-jade-700">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {conViec.map((v) => (
              <div key={v.key} className={cx('flex items-start gap-3 rounded-md p-4 ring-1', TONE[v.tone])}>
      ...
      <h2 className="mb-3 mt-8 text-sm font-semibold uppercase tracking-wider text-muted">Nội dung đang có</h2>
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-8">
        {CARDS.map((c) => (
          <Link key={c.key} to={c.to} className="card p-4 transition hover:-translate-y-0.5 hover:shadow-lift">
            <span className={`mb-3 grid h-10 w-10 place-items-center rounded-md text-white ${c.color}`}>
              <c.icon size={20} />
            </span>
      ...
      {heritagesWithoutCover > 0 && (
        <div className="mt-6 flex items-start gap-3 rounded-md bg-gold-50 p-5 ring-1 ring-gold-200 dark:bg-gold-900/20 dark:ring-gold-800/40">
      ...
      {restaurantsUnverified > 0 && (
        <div className="mt-6 flex items-start gap-3 rounded-md bg-jade-50 p-5 ring-1 ring-jade-200 dark:bg-jade-900/40 dark:ring-jade-700">
      ...
      {chat?.unmatched > 0 && (
        <div className="mt-6 flex items-start gap-3 rounded-md bg-terra-500/10 p-5 ring-1 ring-terra-500/30 dark:bg-terra-500/15">
          <Bot size={22} className="mt-0.5 shrink-0 text-terra-600 dark:text-terra-400" />
          <div>
            <p className="font-semibold text-ink dark:text-jade-50">
